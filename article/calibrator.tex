...
Calibration is related to the optimization process of finding the minimum of a function usually defined as a relation between the desired quantity and the simulated one. There are many fields where calibration is required. This process is more necessary when simplifications on models are done. 
Deterministic models are sensitive to parameters and their ability increases when the quality of the calibration improves [1]. In some way, the model may acquire a predictive character when calibrated. The calibration can be understood as an optimization process where the error between the estimation of the model and the data set is minimized.
On the other hand, there are many phenomena that can be modelled using differential equations (PDEs) that participate in conservation laws. The application of mathematical models to describe physical processes is helpful to perform simulations of possible scenarios using computational facilities. Despite the quality of these mathematical models for predictive purposes, calibration is usually required to provide them useful properties.
The application of the Shallow Water Equations (SWE) for the simulation of open-channel flow, for instance, has been widely used, in particular for irrigation water delivery analysis [2]. To complete the equations with regulation elements, it is accepted that some simplifications can be applied. These simplifications usually include parameters that make possible to model simpler the phenomena that occur on those regulation elements [3].
...
In this work, a descent gradient method is used as example of gradient based optimization. In order to compare this optimizer against a non-gradient based method, the use of Monte Carlo processes by means of [9] as an example of brute force methodology to calibrate is used. Moreover, both  techniques are compared using …
...
simulación requiere $n$ ficheros de entrada.
The structure used by CALIBRATOR is defined in the \emph{main input file}. In
addition, the values which determine the empirical parameters and the
optimization algorithm have to be defined in this very same file. Then,
CALIBRATOR reads the $N\times n$ template files, which are used to build the
simulator input files, substituting labels by empirical parameters created by
the optimization algorithm. The simulator can either directly compare with
...
%%%%%%%%%%%%%%
Para cada experimento se obtiene por tanto un valor objetivo. El valor final de
la función objetivo asociada al conjunto de $N$ experimentos se calcula como la
raíz cuadrada de la media de los cuadrados de los valores objetivo asociados a
cada experimento.
%%%%%%%%%%%%%%
...
}{Diagram of the interactions among CALIBRATOR, the input files and the
...
El formato con el que deben definirse el \emph{main input file} así como el de
...
Debemos crear $N\times n$ template files que servirán para reproducir cada
fichero de entrada asociado a cada experimento. Una vez que el algoritmo de
optimización ha generado un conjunto de $M$ parámetros empíricas, cada uno de
estos template files es analizado sintácticamente por CALIBRATOR para generar
los ficheros de entrada para el programa de simulación. Éstos serán una copia
del template file pero donde se sustituyen las siguientes etiquetas clave:
\begin{description}
\item[@variableX@]: se sustituye por la etiqueta asociada a ese parámetro
	empírico que se define en el \emph{main input file};
\item[@valueX@]: se sustituye por el valor asociado a ese parámetro empírico que
	proporciona el algoritmo de optimización usando el formato definido en el
	\emph{main input file};
\end{description}
donde en ambos casos $X$ corresponde al ordinal del correspondiente parámetro
empírico.
...
machine workings. Nótese que las figuras no se han representado en una escala
uniforme para una mejor visualización de las diferencias entre los resultados
medidos y simulados. El acuerdo entre los resultados medidos y simulados, con
los parámetros calibrados con el algoritmo genético óptimo, debe considerarse
razonable dada la extrema caoticidad del movimiento. En todo caso, se pudo
estimar que la pérdida de uniformidad en el reparto del agua asociada al
movimiento no uniforme de las torres puede considerarse despreciable.
%%%%%%%%%%%%%%%%%%%%%%%%%%%
