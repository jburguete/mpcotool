...
replacing labels by empirical parameter values created by the optimization algorithm.
...
\emph{experimenta data file} to produce the error value. In both cases this error value
is saved in a \emph{objective value file}.
%%%%%%%%%%%%%%
...
Figure~\ref{FigStructure} is a sketch of the structure.
...
The best results where obtained with the genetic algorithms. It is surprising that in this case again the genetic algorithm produces the results with the lowest $R$ coefficients. It is also worth noting that the iterative algorithm improves the results when applied to the Monte Carlo method but it makes things slightly worse in the sweep method.
