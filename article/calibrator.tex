...
calibrations or optimization of empirical parameters required in the formulation of 
numerical simulation models. The structure is such that it can be easily adapted to 
different simulation codes.
One genetic algorithm and two brute force optimization algorithms are included (sweeps and Monte-Carlo).
Parallel computations are enabled in a simple way so that the work load can be distributed among the 
different processors available in a a computer. 
The CALIBRATOR usage and possibilities are illustrated by showing four practical applications in agriculture:
optimization of canal management, calibration of the empirical infiltration coeffcients in surface irrigation,
optimization of a ballistic sprinkler irrigation model and opimization of irrigation engines movement.
...
In the sweep method, first the domain defined by the extreme values of the variables to optimize is divided in regular intervals. Then a simulation is performed using each of the resulting combinations. (NO SE ENTIENDE). The sketch of an example is shown
in figure~\ref{FigSweep}. The total number of required simulations is:
...
They are based on the ideas in \citet{gaul}, but have been fully programmed involving modern external lybraries.
...
Another algorithm is included in GENETIC called "adaptación" although, in the biological sense, it would be better
a smooth mutation. First, one of the variables codified in the genome is randomly selected with uniform probability. Then,
a bit is randonly chosen assuming a probability linearly decresing with the significance of the bit. The new individual receives a copy of the parent's genome with the selected bit inverted. Figure~\ref{FigAdaptation} contains an example.
...
This algorithm is rather similar to the mutation algorithm previously described but, since the probability to affect bits less significativ is larger, so is the probability to produce small changes.
...
\item\citet{gsl}: Scientific library librería required to generate the pseudo-random numbers used by 
...
Once allthe tools installed, the GENETIC source code must be downloaded and it must be compiled following:
...
Some systems (such as Microsoft Windows 64 bits or OpenBSD) might require extra options. In those cases follow the README files in both repositories.
...
differentt simulation codes are presented. The will deal with teh optimizationand/or calibration 
of variables in models used for surface irrigation, open channel flow,sprinckler irrigation or irrigation
engines movement.   

All the cases have been run on a laptop computer equiped with an Intel(R) Core(TM) i7 M620 2.67GHz processor. 
The computation was performed in four parallelized tasks to make the most of the processor's double nucleus with hyperthreading.
...
The canal was initially dry and the simulation was run until a steady state with a discharge of 2 $m^{-1/3}\cdot s$ was reached. 
The objective of this work was to provide the required daily discharge to the three important irrigation gates as well as
to keep a constant discharge of 2 $m^{-1/3}\cdot s$ at the outlet gate. 
...
models used in the model SURCOS \citep{Surcos,SurcosGit,JaviSurcos3} for the simulation of the four furrow fertigation experiments publisehd in \citet{JaviSurcos2}. The furrows were $100$ m long and they were simulated using 100 computatioanl cells per furrow. The furrows were close together, they had the same geometry and the experiments were almost simultaneous, hence the soil properties were assumed uniform. 

The friction model used in SURCOS is the Gauckler-Manning model. The infiltration model is the Kostiakov-Lewis model. They require calibration of three coefficients: the Manning $n$ and the infiltration $K$ and $a$. In the original paper, a brute force method using 3000 simulations was applie, each of them including the four experiments. In the present work, the same number of simulations has been used with the calibration methods implemented in CALIBRATOR. The execution of 3000 simulations in the mentioned laptop computer took about 1h20m.  

Table~\ref{TabSurcos} displays the results obtained for the optimal coefficient valuesand the corresponding
...
\TABLE{\scriptsize}{cccc}
{
	...
	\\ & $R_{adaptation}=0.1$
	\\ \hline
}{Optimal empirical parameters and value of the evaluation function in the simulation of the furrow irrigation cases in \citet{JaviSurcos2} using program SURCOS and several optimization algorithms in CALIBRATOR with the same totalnumber of simulations}{TabSurcos}
...
$k_1$ y $k_2$.
The model is calibrated in two stes: First, the coefficients $D_{50}$, $n$ and $P$ are fitted to adjust the water distribution in windless conditions and are considered representative of the sprinkler model at a given pressure. In a second step, the coefficients $k_1$ and $k_2$ fitting better the water distribution in wind conditions are found.
