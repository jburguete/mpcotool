...
methods may fail due to the location of local minimums. Moreover, stochastic
based optimizations algorithms (as sweep, Monte-Carlo or genetic) are easily
...
\emph{objective value file}. For each experiment, an objective value is
obtained. The final value of the objective function associated with the
...
\emph{main input file} and $o_i$ the objective value obtained by the
simulator/evaluator program. Figure~\ref{FigStructure} is a sketch of the
structure.
