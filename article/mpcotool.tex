...
$N_e$ simulations to estimate the gradient.
...
\TABLE{\scriptsize}{ccc}
{
	...
	SW+CD-2 & SW+GB+CD & $N_x=N_y=40$, $N_{st}=225$, $st_x=st_y=0.01$,
		$rel=1$ \\
	SW+RA-1 & SW+GB+RA & $N_x=N_y=10$, $N_{st}=1200$, $st_x=st_y=0.1$, $rel=1$,
		$N_e=2$ \\
	SW+RA-2 & SW+GB+RA & $N_x=N_y=10$, $N_{st}=600$, $st_x=st_y=0.1$, $rel=1$,
		$N_e=4$ \\
	SW+RA-3 & SW+GB+RA & $N_x=N_y=10$, $N_{st}=240$, $st_x=st_y=0.1$, $rel=1$,
		$N_e=10$ \\
	SW+RA-4 & SW+GB+RA & $N_x=N_y=40$, $N_{st}=450$, $st_x=st_y=0.01$,
		$rel=1$, $N_e=2$ \\
	SW+RA-5 & SW+GB+RA & $N_x=N_y=40$, $N_{st}=225$, $st_x=st_y=0.01$,
		$rel=1$, $N_e=4$ \\
	SW+RA-6 & SW+GB+RA & $N_x=N_y=40$, $N_{st}=90$, $st_x=st_y=0.01$,
		$rel=1$, $N_e=10$ \\
	MC-1 & MC & $N_s=2500$ \\
	...
	MC+RA-1 & MC+GB+RA & $N_s=100$, $N_{st}=1200$, $st_x=st_y=0.1$, $rel=1$,
		$N_e=2$ \\
	MC+RA-2 & MC+GB+RA & $N_s=100$, $N_{st}=600$, $st_x=st_y=0.1$, $rel=1$,
		$N_e=4$ \\
	MC+RA-3 & MC+GB+RA & $N_s=100$, $N_{st}=240$, $st_x=st_y=0.1$, $rel=1$,
		$N_e=10$ \\
	MC+RA-4 & MC+GB+RA & $N_x=1600$, $N_{st}=450$, $st_x=st_y=0.01$, $rel=1$,
		$N_e=2$ \\
	MC+RA-5 & MC+GB+RA & $N_x=1600$, $N_{st}=225$, $st_x=st_y=0.01$, $rel=1$,
		$N_e=4$ \\
	MC+RA-6 & MC+GB+RA & $N_x=1600$, $N_{st}=90$, $st_x=st_y=0.01$, $rel=1$,
		$N_e=10$ \\
...
\TABLE{\scriptsize}{c|cccccc}
{
	...
	SW+IT-1 & 12.8 & 12.4 & 10.2 & 1.5 & -0.7 & 7.4 \\
	SW+CD-1 & {\bf 14.6} & -0.7 & {\bf 14.6} & 11.6 & -1.1 & {\bf 13.5} \\
	SW+CD-2 & {\bf 14.6} & {\bf 14.6} & {\bf 14.6} & 11.5 & 8.1 & {\bf 13.8} \\
	SW+RA-1 & {\bf 13.5,14.2} & -0.7,-0.7 & 13.4,{\bf 13.9} & 0.8,2.9
		& -1.1,-1.1 & 10.8,11.7 \\
	SW+RA-2 & {\bf 13.8,14.1} & {\bf 13.9,14.3} & 13.2,{\bf 14.0} & 2.5,2.8
		& 7.8,8.3 & 3.4,5.5 \\
	MC-1 & 0.7,0.9 & 0.0,0.3 & 0.6,0.8 & 0.4,0.7 & -0.3,-0.2 & 0.6,1.0 \\
	MC+IT-1 & {\bf 13.5,13.8} & 11.9,12.6 & 9.2,10.8 & 2.4,3.4 & -0.5,0.4
		& 0.5,5.9 \\
	MC+CD-1 & {\bf 14.6,14.6} & -0.8,-0.4 & {\bf 14.1,14.3} & 11.7,12.8
		& -1.3,-0.8 & {\bf 13.6,13.8} \\
	MC+CD-2 & {\bf 14.6,14.6} & -0.3,2.9 & {\bf 14.6,14.6} & 8.6,9.7 & -0.6,3.2
		& 13.3,{\bf 13.8} \\
	MC+RA-1 & {\bf 13.8,14.1} & -0.8,2.4 & {\bf 13.5,13.9} & 0.0,1.5 & -1.3,-0.8
		& 11.5-12.1 \\
	MC+RA-2 & 13.4,{\bf 14.0} & -0.3,2.9 & 12.7.5,{\bf 13.6} & 0.6,0.9
		& -0.6,3.0 & 4.4,6.6 \\
	GE-1 & 0.3,1.5 & -0.1,0.4 & -0.1,0.6 & -0.2,0.1 & -1.2,-0.3 & 0.6,0.8 \\
	GE-2 & 0.3,1.2 & -0.4,0.0 & 0.5,0.8 & 0.2,0.7 & -0.8,-0.2 & 0.6,1.3 \\
	GE-3 & 0.3,1.6 & -0.2,0.4 & 0.4,0.8 & 0.1,0.5 & -0.8,-0.1 & 0.6,1.1 \\
	GE-4 & 0.3,1.6 & 0.0,1.0 & -0.0,0.5 & -0.1,0.1 & -0.8,-0.1 & 0.5,0.9 \\
	GE-5 & 0.8,2.2 & 0.1,0.8 & 0.8,1.4 & 0.2,0.5 & -0.9,-0.2 & 1.2,1.6 \\
	GE-6 & 2.2,2.6 & -0.0,0.2 & 1.0,1.6 & 0.3,0.9 & -1.0,-0.1 & 0.9,1.4 \\
	GE-7 & 0.8,1.2 & -0.3,0.1 & 0.5,0.9 & 0.2,0.5 & -0.9,-0.5 & 0.9,1.4 \\
	GE-8 & 1.7,2.0 & 0.3,0.7 & 1.2,1.6 & 1.0,1.1 & -0.4,-0.0 & 1.2,1.6 \\
	GE-9 & 1.0,1.4 & 0.1,0.5 & 0.9,1.1 & 0.5,0.8 & -0.9,-0.5 & 1.0,1.6 \\
	GE-10 & 1.4,1.6 & 0.1,0.5 & 1.0,1.4 & 0.8,0.9 & -0.5,-0.2 & 1.2,1.3 \\
	GE-11 & 1.4,1.7 & 0.1,0.6 & 1.0,1.3 & 0.7,1.1 & -0.8,-0.2 & 1.0,1.5
