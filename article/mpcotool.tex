...
\subsection{Gradient based method (GB)}
...
relaxation parameter, the gradient based method checks $N_e$ variable
...
\begin{description}
\item[Coordinates descent (CD)]: this method builds the $\vec{t}_j$ vectors by
increasing or decreasing only one variable:
\[
	\vec{t}_1=\MATRIX{c}{st_1\\0\\\vdots\\0},\quad
	\vec{t}_2=\MATRIX{c}{-st_1\\0\\\vdots\\0},\quad
	\vec{t}_3=\MATRIX{c}{0\\st_2\\\vdots\\0},\quad
	\vec{t}_4=\MATRIX{c}{0\\-st_2\\\vdots\\0},
\]
\EQ
{
...
variables number $N_v$:
\EQ{N_e=2\,N_v}{EqNestimatesDescent}
\item[Random (RA)]: the vectors $\vec{t}_j$ are built randomly as:
\EQ
{
...
\EQ{J=\sqrt{\frac{1}{N_{exp}}\,\sum_{i=1}^{N_{exp}}\ABS{w_i\,o_i}^2},}
{EqObjectiveFunction}
...
\end{itemize}
En la tabla~\ref{TabOriginalTests} se muestran todas estas funciones con su
mínimo absoluto y el dominio usado para la optimización.
...
\item[$d_i=\sqrt{\PA{\tilde{x}_i-\overline{x}}^2
	...
	simulaciones realizadas y el valor óptimo de los parámetros (para una
	optimización en 2 variables).
\end{description}

En la figura~\ref{FigTests} se muestra una representación del valor de estas
funciones en el plano $x-y$.
\FIGVI{Sphere.eps}{Ackley.eps}{Booth.eps}{Rosenbrock.eps}{Easom.eps}{Beale.eps}
{Mapa en el plano $x-y$ de los valores de las funciones objetivo usadas como
test}{FigTests}
En la figura~\ref{FigTestsZoom} se muestra un zoom de alguna de estas funciones
para resaltar sus peculiaridades.
\PLOTII{Ackley2.eps}{Easom2.eps}{A zoom of (left) Ackley and (right) Easom
objective functions}{FigTestsZoom}

A continuación vamos a ver la capacidad de los diferentes algoritmos con
para obtener el óptimo en todos estos tests usando un número fijo de
simulaciones total de 2500 en cada método. Para simplificar las diferentes
combinaciones de algoritmos de optimización y parámetros de configuración se van
a usar las abreviaturas de la tabla~\ref{TabMethods}.
\TABLE{\scriptsize}{ccc}
{
	Abreviaturas & Algorithms & Parameters \\
	\hline
	SW-1 & SW & $N_x=N_y=50$ \\
...
}{Abreviaturas de los métodos utilizadas en los resultados de los tests}
{TabMethods}

A continuación examinamos el comportamiento de los diferentes algoritmos de
optimización en la sphere function, una función simétrica, convexa, sin extremos
locales y fácil de optimizar.

En la figura~\ref{FigSphereVariables} se representan todas las combinaciones de
variables simuladas por los distintos algoritmos para ilustrar la forma de
operar de cada método. Se observa la forma regular de explorar el espacio de
variables del método de barrido frente a la forma aleatoria del de Monte-Carlo.
Puede verse también cómo en ambos casos el algoritmo iterativo va reduciendo
progresivamente el intervalo de búsqueda. El método genético también realiza una
búsqueda aleatoria pero progresivamente va buscando con mayor probabilidad en
puntos cercanos al óptimo. En cambio el método basado en gradiente busca
rápidamente en la dirección del óptimo desde el valor mejor proporcionado por el
método de fuerza bruta asociado.
...
{Variables combination simulated by diferent algorithms}{FigSphereVariables}

En la figura~\ref{FigSphereSWMC} se muestra la convergencia obtenida con los
algoritmos de fuerza bruta de barrido y de Monte-Carlo y la combinación de ambos
con el algoritmo iterativo. Se comprueba que cómo la convergencia aumenta
con el algoritmo iterativo. Aunque teóricamente la convergencia con los métodos
iterativos aumenta, con bajos $N_b$ y $tol$ el valor óptimo puede caer fuera del
intervalo de búsqueda y arruinar la convergencia, como sucede con SW+IT-2 y
MC+IT-2.
\PLOTII{sphere-evolution-sw.eps}{sphere-evolution-mc.eps}
{Convergencia de la optimizacion de $g_{Sphere}$ con (left) el algoritmo de
barrido and (right) Monte-Carlo combinados ambos con el algoritmo iterativo y
distintos parametros}{FigSphereSWMC}

En la figura~\ref{FigSphereSWMCGR} se muestra la convergencia obtenida con los
algoritmos de fuerza bruta de barrido y de Monte-Carlo acoplados al algoritmo
basado en gradiente. Se comprueba que el algoritmo basado en gradiente tiene una
convergencia muy superior, por lo que en esta función la convergencia es mayor
cuanto menor es el número de casos simulados con el algoritmo de fuerza bruta.
En cuanto al método de estimar el gradiente, el método aleatorio se muestra
superior al método de descenso de coordenadas.
\PLOTII{sphere-evolution-sw-cdr.eps}{sphere-evolution-mc-cdr.eps}
{Convergencia de la optimizacion de $g_{Sphere}$ con (left) el algoritmo de
barrido and (right) Monte-Carlo combinados ambos con el algoritmo basado en el
gradiente y distintos parametros}{FigSphereSWMCGR}

En la figura~\ref{FigSphereMCCDR} se muestra la influencia de los distintos
parámetros en la convergencia lograda por el método basado en gradiente.
Arriba a la iquierda se ilustra el efecto del parámetro $rel$. Con un tamaño de
paso pequeño, valores de $rel$ nulos producen una convergencia menor. Arriba a
la derecha puede verse el efecto del tamaño de paso. En general, debe escogerse
un tamaño de paso unas pocas veces menor que la distancia aproximada esperada
de la distancia entre el óptimo y el mejor punto obtenido por el método de
fuerza bruta. Finalmente, en la figura de abajo se comprueba que el menor número
de estimaciones del gradiente ($N_e=2$) proporcionan la convergencia más rápida
con el método aleatorio de estimar el gradiente.
\FIGIII{sphere-evolution-mc-cd-r.eps}{sphere-evolution-mc-cd-s.eps}
{sphere-evolution-mc-r.eps}
{Convergencia de la optimizacion de $g_{Sphere}$ con el algoritmo de Monte-Carlo
y el método basado en gradiente configurado con diferentes combinaciones de
parametros}{FigSphereMCCDR}

En la figura~\ref{FigSphereGE} se presentan las convergencias del algoritmo
genético con 11 diferentes combinaciones de parámetros. Los resultados, aunque
mejoran ligeramente los de los algoritmos puros de fuerza bruta, son mucho
peores que los del algoritmo iterativo y los del método basado en gradiente.
\FIGIII{sphere-evolution-ge-1-4.eps}{sphere-evolution-ge-5-8.eps}
{sphere-evolution-ge-9-11.eps}{Convergencia de la optimizacion de $g_{Sphere}$
con el algoritmo genético y distintas combinaciones de parametros}{FigSphereGE}

Finalmente, en la tabla~\ref{TabTestResults} se muestran los resultados
obtenidos por 23 combinaciones de algoritmos y parámetros en las 6 funciones
test. El resultado se presenta como menos el logaritmo decimal de la distancia
obtenida al punto óptimo. Este valor viene a suponer el número de dígitos
decimales de precisión obtenidos. Se han resaltado valores próximos a 14 que es
la precisión teórica de la máquina. Los resultados de GE pueden calificarse como
decepcionantes, si bien logran una ligera ventaja en una función convexa como la
función esférica su ventaja sobre la fuerza bruta resulta más que dudosa en los
demás casos En cuanto a los métodos de fuerza bruta SW a funcionado ligeramente
mejor que MC. No obstante, podría tratarse de un hecho aleatorio. De hecho,
teóricamente, MC saca ventaja en situaciones con muchas variables y en los que
la influencia de cada variable sobre la función objetivo es distinta, cosa que
no ocurre en los tests analizados. El algoritmo iterativo de MPCOTool sí que ha
supuesto una mejora importante, logrando una sustancial mejora sobre la fuerza
bruta en práctimente todos los casos, aunque ha tenido más problemas con la
función de Rosenbrock. Ha funcionado ligeramente mejor acoplado a MC, aunque
también podría ser un hecho aleatorio dado las funciones analizadas. Aunque la
convergencia en la función esférica de GB es mayor usando un número bajo de
simulaciones del método de fuerza bruta asociado, se revela como una mala
estrategia en situaciones con muchos mínimos locales. En este caso funciona
mejor que el método estocástico realice un número suficiente de simulaciones
para obtener un punto cercano al óptimo y usar entonces GB para que converja
rápidamente. En cuanto al método de estimar el gradiente, aunque RA usado con
un número bajo de estimaciones acelera la convergencia, falla en algunos casos a
la hora de encontrar el óptimo. CD resulta un método algo más lento pero mucho
más seguro.
