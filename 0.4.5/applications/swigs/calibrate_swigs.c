...
int main(int argn, char **argc)
{
	...
	printf("calibrate_swigs %s %s-o\n", argc[1], argc[2]);
	snprintf(buffer, 512, "mkdir %s-o", argc[2]);
	system(buffer);
	snprintf(buffer, 512, "cp %s %s-o/case.xml", argc[1], argc[2]);
	system(buffer);
	snprintf(buffer, 512, "cp simulate.xml %s-o", argc[2]);
	system(buffer);
	snprintf(buffer, 512, "cd %s-o; ../swigsbin simulate.xml", argc[2]);
	system(buffer);
	snprintf(buffer, 512, "%s-o/contributions", argc[2]);
	...
	snprintf(buffer, 512, "ls %s-o", argc[2]);
	system(buffer);
	snprintf(buffer, 512, "%s-o/overflow", argc[2]);
	...
	snprintf(buffer, 512, "rm -rf %s-o", argc[2]);
	system(buffer);
