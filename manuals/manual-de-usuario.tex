...
\begin{figure}[ht!]
	\centering
	\includegraphics[width=10.13cm]{calibrator-es.eps}
...
\begin{figure}[ht!]
	...
	\caption{Cuadro de diálogo de los resultados óptimos de la aplicación con
		interfaz gráfica de usuario interactiva de Calibrator.\label{FigResult}}
\end{figure}
...
La estructura seguida por Calibrator se resume en el \emph{fichero de entrada
...
de optimización escogido.
Then, Calibrator reads the
$N_{experimentos}\times N_{entradas}$ templates to build the simulator input files
...
experimentos is calculated by means of:
\EQ
{
	J=\sqrt{\frac{1}{N_{experimentos}}
	\,\sum_{i=1}^{N_{experimentos}}\ABS{w_i\,o_i}^2},
}{EqObjectiveFunction}
